...
:BuildExample
    ...
    set EXAMLE_PJ_FILE=%EXAMPLE_ROOT_DIR%/%EXAMPLE_NAME%.jucer
    ...
    if %ERRORLEVEL% neq 0 exit /b %ERRORLEVEL%
    powershell -Command "(Get-Content '%EXAMLE_PJ_FILE%.jucer') -replace '<JUCERPROJECT ', '<JUCERPROJECT cppLanguageStandard=\"17\" ' | Set-Content '%EXAMLE_PJ_FILE%'"
    if %ERRORLEVEL% neq 0 exit /b %ERRORLEVEL%
